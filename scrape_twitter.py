...
async def scrape_tweets(client, search_query, tweet_limit):
    tweet_count = 0
    tweets = None
    ...
    while tweet_count < tweet_limit:
        ...
        for tweet in tweets:
            ...
            with open('social_media_scraper/tweets.csv', mode='a', newline='', encoding='utf-8') as csvfile:
...
async def twitter_scrape(search_query):
    ...
    config = ConfigParser()
    config.read('social_media_scraper/twitter_config.ini')
    ...
    client.save_cookies('social_media_scraper/cookies.json')
    client.load_cookies('social_media_scraper/cookies.json')

    # scrape tweets
    await scrape_tweets(client, search_query, limit)
